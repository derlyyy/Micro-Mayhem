...
public class PlayerCollision : MonoBehaviour
{
	...
	private void FixedUpdate()
	{
		...
		float num = cirCol.radius * base.transform.localScale.x * 1.2f;
		...
		for (int j = 0; j < array2.Length; j++)
		{
			...
			if (vel.rb.simulated || !vel.rb.isKinematic)
			{
				...
				vel.rb.velocity += normalized * value * 10f * Time.timeScale;
				vel.rb.velocity -= vel.rb.velocity * value * 1f * Time.timeScale;
			}
	...
	{
		if (!(Vector2.Angle(data.playerVel.rb.velocity, hit.normal) < 90f) && !isBounce && data.playerVel.rb.velocity.magnitude > bounceTreshold)
		{
